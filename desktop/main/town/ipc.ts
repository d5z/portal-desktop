...
export function registerTownIpc(options: TownIpcOptions) {
  ...
  handle('beings:town-open', async (route: string) => {
    if (typeof route !== 'string' || !/^\/(?:grove|api\/(?:[a-z]+\/help|announcements\/[a-zA-Z0-9_-]{1,160}|grove\/[a-zA-Z0-9_-]+\/download)|(?:embers|scrolls|seeds)\/[a-zA-Z0-9_-]{1,160})?$/.test(route)) throw new Error('不支持的 Town 链接。');
